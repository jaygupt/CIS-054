 ...
 * StudentID: 0961979
 * Date: 3/24/2020
 * Program Name: TaxEstimator.cpp
 * CIS054 C/C++ Programming 
 * 
 * Inputs: Tax Status, # of Exemptions, Total Income, Deductions, $ Witheld
 * Outputs: Income, $ in Exemptions, $ in Deductions, 
 * Taxable Income, Tax Estimate, Refund/$$ Due 
*/
#include <iostream>
#include <iomanip>
using namespace std;
...
int inputExemptions();
double inputIncome();
double inputDeductions(int);
double standardDeduction(int);
...
int main(int argc, char* argv[])
{
    ...
    cout << "FEDERAL TAX ESTIMATOR" << endl;
    cout << "The data in this project is from a previous tax year. Tax tables" << endl;
    cout << "are adjusted each year due to inflation and legal changes.  The" << endl;
    cout << "output from this program is for information purposes only." << endl << endl;
    do
    {
        // input the data
        status = inputStatus();  // input the tax payer status (1-5)
        if (status == 0)
            break;   // end the program

        exemptions = inputExemptions();
        totalIncome = inputIncome();
        deductions = inputDeductions(status);  // taxpayer status to inputDeductions
...
{
    int exemptions;
    cout << "Enter the number of exemptions: (you + dependents): "; 
    cin  >> exemptions;
    return exemptions;
}
...
{
    char typeOfDeduction; 
    // ask user to select either standard deduction or itemized deduction
    do {
        cout << "Please select either standard deduction (s) or itemized deduction (i): "; 
        cin >> typeOfDeduction; 
    } while (typeOfDeduction != 'i' && typeOfDeduction != 'I' && typeOfDeduction != 's' && typeOfDeduction != 'S'); 

    double deductions = 0; 
    if (typeOfDeduction == 'i' || typeOfDeduction == 'I') {
        cout << "Enter the amount of your Itemized Deductions: ";
        cin  >> deductions;
    } else if (typeOfDeduction == 's' || typeOfDeduction == 'S') {
        deductions = standardDeduction(taxpayerStatus - 1); 
    }
    return deductions; 
}

double standardDeduction(int index) {
    double standardDeductions [5] = {6300.00, 12600.00, 6300.00, 9250.00, 12600.00};  

    return standardDeductions[index]; 
}

// The asterisk * indicates that a pointer is to be returned.
taxTable *tableSelection (int s)
{
    const double maxDouble = 1E12;	// just some big number = 1 * 10^12
        //from      to          tax         percent
